(function(win){
    ...
    // 绘制缺口
    function drawCanvas(ctx,operation,x,y){
        ctx.beginPath();
        ctx.moveTo(x,y);//方块左上角
        ctx.lineTo(x+l/2,y);
        ctx.arc(x+l/2,y-r+2,r,0,2*PI);//圆形向下移动2
        ctx.lineTo(x+l/2,y);
        ctx.lineTo(x+l,y);
        ...
        // ctx.arc(x+r-2,y+l/2,r,0,2*PI);
        ctx.globalCompositeOperation='xor';//交叉啃掉
        ctx.fill();
    }
    ...
    class captcha{
        ...
        // 绘画
        draw(){
            this.x=getRandomNumberByRange(ll+10,w-(ll+10));//右边10下边10
            this.y=getRandomNumberByRange(10+r*2,h-(ll+10));
            drawCanvas(this.canvasCtx,'fill',this.x,this.y);
            drawCanvas(this.blockCtx,'clip',this.x,this.y);
        }
        ...
        // 绑定事件
        bindEvents(){
            this.el.onselectstart=()=>false;//到底有啥用呢原来是阻止双击选中
            ...
            document.addEventListener('mousemove',(e)=>{
                ...
                var blockLeft=(w-ll+2)/(w-40)*moveX;
